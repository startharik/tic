...
const LiveTrackingPage: React.FC = () => {
  ...
  const selectedUser = useMemo(
    () => {
      if (selectedId) {
        return onlineUsers.find((e) => e.id === selectedId) ?? onlineUsers[0];
      }
      return onlineUsers[0];
    },
    [selectedId, onlineUsers],
  );
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Live User Tracking</h1>
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {onlineUsers.length === 0 && (
              <div className="p-6 text-sm text-slate-500">
                No users online yet. Open the mobile app and start a job to begin sending live GPS.
              </div>
            )}
            {onlineUsers.map((user) => (
              <div
                ...
                onKeyDown={(e) => (e.key === 'Enter' ? setSelectedId(user.id) : null)}
                className={`p-4 border-b border-slate-50 cursor-pointer transition-colors group ${
                  selectedId === user.id ? 'bg-primary-50' : 'hover:bg-slate-50'
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-slate-900 group-hover:text-primary-600 transition-colors">{user.name}</h4>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{user.status}</p>
                ...
                <div className="mt-3 space-y-1.5">
                  ...
                  <div className="flex items-center text-xs text-slate-500">
                    ...
                    <span>Last seen: {user.lastUpdate}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="flex-1 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden relative">
          <MapContainer
            center={selectedUser ? [selectedUser.lat, selectedUser.lng] : defaultCenter}
            zoom={selectedUser ? 10 : 6}
            scrollWheelZoom
          ...
          >
            ...
            />
            {selectedUser && <FlyTo center={[selectedUser.lat, selectedUser.lng]} zoom={10} />}
            <MapControls />
            {onlineUsers.map((user) => (
              <Marker key={user.id} position={[user.lat, user.lng]} icon={engineerMarkerIcon}>
                <Popup>
                  <div className="font-semibold">{user.name}</div>
                  <div className="text-sm">{user.role.replace('_', ' ')}</div>
                  <div className="text-sm">{user.location}</div>
                  <div className="text-xs text-slate-600">Status: {user.status}</div>
                  <div className="text-xs text-slate-600">Job: {user.job}</div>
                </Popup>
              </Marker>
            ))}
          </MapContainer>
            ...
            <div className="absolute left-4 bottom-4 z-[500] bg-white/90 backdrop-blur px-3 py-2 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-bold text-slate-900 truncate">{selectedUser.name}</div>
                  <div className="text-xs text-slate-500 truncate">{selectedUser.location}</div>
                </div>
              </div>
            </div>
          )}
